# Basis International School Shenzhen Developers' Club New Club Site
- to be deployed at [new.biszweb.club](new.biszweb.club) once basically done
- replacing [www.biszweb.club](www.biszweb.club) when fully complete (hopefully around 2026Q2)
## Development
### How to run this locally:
Follow these steps to run the project locally and develop and test it:
#### 1. Install Dependencies
In your project directory, run (make sure you have the newest version of Node.js):
```bash
npm install
```
This installs all the necessary packages in package.json.
#### 2. Starting the Server
After installation, start the server with:
```bash
npm run dev
```
The page would then be available through port [http://localhost:5173](http://localhost:5173). You could then open it and view any page to check functionality.